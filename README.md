...
## The "Win Condition" line could name a move nobody's build actually had selected, and Core Four's real limitation explained (Milestone 67)

Phoenix pasted a real Battle Plan report and flagged two things: "It hasnt taken into account farigirafs trick room set up - is this intentional?" and "Amphoras also doesnt know electric terrain - and its not a valid move change to do it mid battle."

**The Trick Room question: a real, documented limitation, not a bug.** Battle Plan's Core Four (`wcPickBestLineup` -> `wcRankLineupsHeuristic`, both in strategy.js since Milestone 56) enumerates every real C(6,4)/C(6,3) combination and scores each purely by averaging `wcScoreMatchup` per member against the reference threats, plus an optional team-wide synergy bonus that `wcPickBestLineup`'s own call always passes as 0. There is no factor anywhere in that scoring for "does this specific combination support a shared archetype like Trick Room together" -- it's individual matchup strength only. Farigiraf lost its spot on pure per-member matchup numbers, with no credit for the Trick Room synergy it would have brought the rest of the lineup. Confirmed by reading the real scoring code, not assumed -- this is a genuine, existing gap in Core Four's selection, not something this milestone changes.

**The Electric Terrain question: a real bug, now fixed.** Every archetype candidate in `wcBuildStrategyCandidates` (Trick Room, Tailwind, weather, terrain, screens, hazards, Wide/Quick Guard, Safeguard, redirect, Helping Hand -- all 13 blocks) was built entirely from `canLearn(pokemon, move)`, a check against the species' full theoretical learnset (`data/learnsets.json`) -- exactly right for the team-BUILDING suggestion engine this function also feeds (Auto-build, Dream Team, "you could teach it this"), but wrong for the "Win Condition" line in Team Strategy Report, Rival Breakdown, and Battle Plan, all three of which describe an imminent battle with an already-fixed team. Ampharos genuinely can learn Electric Terrain as a species; its actual built moveset didn't have it selected. The report presented that hypothetical as the team's executable win condition anyway -- a move change that isn't legal mid-battle, exactly what Phoenix caught.

**The fix, scoped the way Phoenix chose when asked ("prefer real, already-built archetypes; else say 'no shared win condition'").** A new `wcCandidateIsReal(setter, wantMoves, builds)` checks the setter's REAL build (not the species learnset) for every one of an archetype's defining moves; ability-only archetypes (weather from Drought/Drizzle, Sand Stream/Snow Warning) are always real since an ability needs no move slot. Setter *selection* itself now prefers a genuinely-already-built member over a merely-learnable one at all 13 candidate sites (`wcRealSetterFor`/`wcRealSetterForAny`) -- not just a label applied after the fact, since a pool can contain both a real setter and a stronger-looking merely-learnable one, and the old fastest/slowest/strongest-pick fallback could easily land on the wrong one. A confirmed-real candidate's note is rephrased from "X can learn Y" to "X already knows Y" (`wcRephraseNoteAsReal`) so the language matches what's actually true. `wcAnalyzeTeamStrategy` (shared by all three reports) now only ever selects its winner and alternative from real candidates; when nothing on the roster is genuinely already built for any archetype, it falls back to the existing "independent attackers" read with an honest explanation -- distinguishing a team where notes ruled every archetype out, one where a move is learnable somewhere but nobody has it built, and one where nothing fits at all. `wcGenerateTeamBuilds`/Auto-build/Dream Team are unaffected -- they consume `wcBuildStrategyCandidates`'s full candidate list directly, not the `isReal`-filtered result, since a hypothetical "you could build this in" suggestion is exactly what they're supposed to make while a team is still being constructed.

**Verification.** `tools/test-terrain-archetypes.mjs` gained a fixture pair reproducing Phoenix's exact report: with Pikachu's build actually running Electric Terrain, it correctly wins with "Pikachu already knows Electric Terrain" and no "can learn" language; with the same fixture but nobody's build actually holding the move (Phoenix's real scenario), Electric Terrain is correctly excluded and the report falls back to "independent" with the new honest explanation. `tools/test-chilly-reception-and-parting-shot.mjs` and `tools/test-screens-archetype.mjs` fixtures updated so their setters' builds genuinely hold the moves those tests are meant to exercise (previously only learnable, not built -- the exact pattern this milestone fixes). Checked the Tailwind archetype's pivot-sequencing sub-clause specifically, since it's the one candidate note with two separate "can learn" phrases in the same string (the primary Tailwind clause and an optional "it can also learn a pivot move" aside) -- confirmed by direct execution that rephrasing only ever touches the primary, already-real clause and correctly leaves a still-hypothetical pivot suggestion phrased as hypothetical. Full `tools/test-*.mjs` suite (44 files, excluding the pre-existing, sandbox-only `test-weight-search.mjs` file-deletion permission issue unrelated to this change) green throughout.

## Regulation M-C data audit: missing signature moves, ability-options gap, and two patch-note fixes (Milestone 68)

Phoenix asked for a full fresh re-check of the whole roster's movesets, stats, and items against Bulbapedia/Serebii/Limitless/other sources, five days after Regulation M-C's September 9 launch — not trusting that Milestone 66's Day-1 pass caught everything.

**Base stats: re-verified, zero errors found.** Spot-checked all 34 Regulation M-C species/Mega forms plus a 13-species sample of the pre-existing roster (Garchomp, Gengar, Tyranitar, Dragonite, Metagross, Greninja, Gholdengo, Vileplume, Qwilfish, Scizor, Aggron, Mega Aggron, Archaludon) against known-correct values. Every base stat spread matched exactly — this file is solid.

**Movesets: real, confirmed gaps, not a hallucination.** Fresh research (live WebSearch/WebFetch, September 2026) turned up Version 1.2.0's actual patch notes — a real balance pass on top of the Milestone 61/63 roster work, never previously reconciled against `data/moves.json`/`data/learnsets.json`. Cross-checked every claim against 2+ independent sources (pokemondb.net + Bulbapedia, the same discipline Milestone 63 established) after catching the exact same fabricated ability, "Cacophony" for Mabosstiff's hidden slot, that Milestone 63 already flagged and discarded once — a good sign the AI-summarized layer these fetches go through still needs independent corroboration every time, not just once.

Fourteen real moves tied to Regulation M-C's new roster were entirely missing from `moves.json`, and every one of them turned out to be that species' actual signature or standout move, absent from its `learnsets.json` entry too: Cinderace was missing both **Pyro Ball** and **Court Change**; Baxcalibur and Mega Baxcalibur were missing **Glaive Rush**; Pawmot was missing both **Double Shock** and **Revival Blessing**; and Rillaboom (**Drum Beating**), Inteleon (**Snipe Shot**), Mabosstiff (**Jaw Lock**), Sirfetch'd (**Meteor Assault**), both Toxtricity forms (**Overdrive**), Grapploct (**Octolock**), and Pincurchin (**Zing Zap**) were each missing their own. Gogoat was missing **Milk Drink** from its learnset — the move existed in `moves.json` already (pre-dating this milestone) but with the wrong PP (8 instead of the real, current-gen 5), fixed here too. All 14 new move entries got real power/accuracy/PP/category/type data, confirmed against both sources before shipping.

**Two direct Version 1.2.0 patch-note changes, applied.** The update also reduced **Wish** and **Strength Sap** from 12 PP to 8 PP (confirmed via two independent patch-note write-ups), and removed **Mirror Coat** and **Metal Burst** from Archaludon's movepool specifically (both moves remain legal for every other Pokémon that learns them — this is a per-species removal, not a global ban). Both applied directly to `moves.json`/`learnsets.json`. The same patch notes said Politoed "can no longer use Pound" — checked, and Politoed's `learnsets.json` entry never had Pound in the first place, so no change was needed there.

**Slash, added but only partly wired in.** The patch notes also said "Slash becomes usable," with no further detail. Added the move itself (real mainline stats: Normal, Physical, 80 power, 100 accuracy, 20 PP, high critical-hit ratio) and wired it into Persian and Alolan Persian's learnsets, since both are confirmed real Slash-learners and both are newly-legal Regulation M-C additions — the most likely reason this specific move would suddenly need to exist in Champions data at all. **Not done:** checking Slash-eligibility for the rest of the 333-entry roster (Scyther, Sneasel, Absol, and others that also learn it in the mainline games) — that's a real, open gap, not a hidden one; flagging it here rather than guessing which of them Champions considers legal Slash-users.

**A structural gap unrelated to the moveset research: `data/ability-options.json` never got extended to any of Milestone 63's 28 base-form Regulation M-C additions.** `abilities.json` (the single "standout ability" per species that Auto-build etc. actually use) was already complete for all 333 roster entries — but `ability-options.json` (the full selectable regular+hidden ability list the Team Builder's ability dropdown reads) silently stopped at the pre-M-C roster. Every Mega Evolution correctly has no entry here (a Mega's ability is fixed, not selectable — confirmed this is by design, not a gap, before touching anything), but all 28 new base-form species/forms did need one. Filled in real ability lists (regular abilities plus hidden, each flagged) for Wigglytuff, Persian, Alolan Persian, Farfetch'd, Sirfetch'd, Mr. Mime, Swalot, Salamence, Gogoat, Golisopod, Rillaboom, Cinderace, Inteleon, Thievul, both Toxtricity forms, Grapploct, Perrserker, Pincurchin, both Indeedee genders, Arboliva, all four Squawkabilly plumage forms, Mabosstiff, and Baxcalibur — cross-checked against pokemondb.net for each, catching the Cacophony fabrication along the way (Mabosstiff's real hidden ability is Stakeout, not Cacophony, confirmed by two sources and consistent with the single-ability pick `abilities.json` already had).

**Items: re-checked, already correct.** All 18 Regulation M-C items (12 held items plus the 6 new Mega Stones) were already present in `items.json` with accurate descriptions — nothing to fix here, Milestone 61/63's work held up.
...
## The item picker was suggesting items that aren't actually legal in Pokemon Champions right now (Milestone 70)

Phoenix reported "assault vest isnt avaialable," then clarified: **"i mean that its not a legal itm"** -- Assault Vest's data (name, description) was accurate, but it isn't currently a usable held item in the real game at all, and WinCon's item picker offered it anyway. Given the choice between a one-item patch or a full audit, Phoenix chose the full pass.

**The real scope was much bigger than one item.** `data/items.json` is a comprehensive mainline-Pokemon items database -- 583 entries covering every held item, Mega Stone, berry, Poke Ball, evolution stone, fossil, and Z-Crystal ever printed -- with no concept of which of those are actually implemented in Pokemon Champions, a live-service game that deliberately ships a much smaller, curated held-item pool. `builder.js`'s item-picker datalist and `strategy.js`'s Auto-build/Dream Team item-recommendation pools both drew from the full 583-entry list with zero filtering, so long-standing VGC staples that Champions simply doesn't currently support -- Choice Band, Choice Specs, Assault Vest, Safety Goggles, Weakness Policy, Heavy-Duty Boots, Utility Umbrella, and more -- could be picked, suggested, or auto-assigned to a team exactly as if they were real, legal choices.

**Researched the actual legal pool directly, not summarized.** Serebii's dedicated Pokemon Champions items page (`serebii.net/pokemonchampions/items.shtml`) lists the game's real, current held-item catalog in three sections -- Hold Items, Mega Stones, and Berries -- separately from a fourth "Miscellaneous" section of non-battle tickets/consumables that was excluded outright. A first WebFetch pass truncated mid-list on a long combined fetch; re-pulled with the Claude-in-Chrome browser's raw-DOM text extraction instead (no AI-summarization step to truncate or hallucinate) to get the complete, exact list: 57 Hold Items, 81 Mega Stones (including Champions-specific dual variants like Absolite Z/Garchompite Z/Lucarionite Z alongside the mainline X/Y pairs), and 28 Berries -- 166 legal items total, cross-checked against `data/item-effects.json` (the items the battle engine mechanically implements) and against pokemon-zone.com's per-item tournament-usage pages, which independently corroborated Life Orb's legality (real Regulation M-C usage stats) and the absence of the disputed items (404s on their individual pages).

**The fix.** All 166 confirmed-legal entries in `data/items.json` are now marked `"legal": true`; the other 417 (including every non-battle item and every currently-unsupported held item) are left exactly as they were, so the full mainline database stays intact for reference and for whenever a future Regulation actually adds them. `builder.js`'s `ensureItemDatalist()` now filters to `legal === true` before building the item picker's suggestion list, so the picker can no longer surface an item that isn't actually usable. `strategy.js`'s `WINCON_ITEM_POOLS` (both Doubles/Singles bulky pools, and the fast-Physical/fast-Special pools) and `WINCON_ITEM_FALLBACK_POOL` had every illegal entry removed and backfilled with legal substitutes (Mental Herb/White Herb/Shell Bell/Muscle Band/Wise Glasses) so Auto-build and Dream Team can never hand out an item nobody can actually hold, and pool sizes stay healthy. The Sun/Rain archetype counters' "or a Utility Umbrella holder" advice (an illegal item) was replaced with the real, legal alternative -- a Cloud Nine/Air Lock ability holder, which negates weather outright -- and the Hazards counter's "or a Heavy-Duty Boots holder" clause was dropped, leaving just Rapid Spin/Defog. `wcAntiTrickRoomAudit`'s fourth check (a Safety Goggles holder) was removed entirely, since that defense can never actually exist on a legal team; the audit now honestly reports on the three real tools (Taunt, Fake Out, a minimum-Speed pivot) instead of a fourth that can never fire. `WINCON_HIGH_VARIANCE_ITEMS` dropped its "Lax Incense" entry for the same reason. The battle simulator engine itself (`battle-sim-engine.js`) was deliberately left untouched -- it's correct for it to still mechanically model any item's real effect if one ever ends up in a build (a manually-typed legacy value, or a future Regulation re-adding it), since simulating mechanics and recommending legal choices are different jobs.

**Verification.** `tools/test-meta-analyst.mjs`'s `wcAntiTrickRoomAudit` fixtures updated from four tools/confirmations to the real three, matching the removed Safety Goggles check. Full `tools/test-*.mjs` suite (44 files) green throughout, including the updated fixtures.
