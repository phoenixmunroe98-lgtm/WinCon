...
Updated `tools/test-meta-analyst.mjs` (now 27 checks) for the new report shape: `wcActiveArchetypesForBuiltTeam` and `wcBestMatchupAnalysis` are tested directly (including the real four-archetype Gemini-team case and an ability-only Sand Stream detection), and the report-shape checks confirm `report.bestMatchup`/the new "Best Matchup Against This Strategy" mode are present while the old "Anti-Trick-Room Mode" is gone. All 31 test files green.
